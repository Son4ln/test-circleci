...
class S3UploaderController extends Controller
{
    ...
    */
    public function __construct(Request $req){
        ...
        $this->clientPrivateKey      = config('filesystems.disks.s3.client_secret');
        $this->serverPublicKey       = config('filesystems.disks.s3.key');
        $this->serverPrivateKey      = config('filesystems.disks.s3.secret');
        $this->expectedBucketName    = $bucket;
        $this->expectedHostName      = 'https://' . $bucket . '.s3-'. $region . 'amazonaws.com';
        $this->expectedMaxSize       = 5368709120;
        $this->expectedBucketRegion  = $region;
        $this->expectedBucketVersion = 'latest';
        $this->req                   = $req;
    }
    ...
    {
        if (preg_match('/creative-rooms\/\d/', request()->headers->get('referer')) && $request->has('room')) {
            $room = \App\CreativeRoom::findOrFail($request->input('room'));
    ...
    public function deleteEndpoint($uuid)
    {
        return response('200');
    }
    ...
    private function isPolicyValid($policy)
    {
        ...
        return $bucket == $this->expectedBucketName;
    }
