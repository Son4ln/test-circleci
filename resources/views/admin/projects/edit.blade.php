...
@section('content-header')
  <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
    <h4 class="page-title">@lang('admin.projects.edit_title')</h4>
  </div>
@endsection
...
<div class="white-box">
  @if (!$project->duedate_at)
    @php $project->is_duedate_undecided = 1 @endphp
  @endif
  {{ Form::model($project, [
    'url' => route('admin.projects.update', ['id' => $project->id]),
  ...
  <div class="form-group">
    ...
    <div class="col-md-6 {{ $errors->has('part_of_work') ? 'has-error' : '' }}">
      @foreach ($part_of_work as $key => $value)
        ...
        <label for="part-of-work-{{ $key }}" class="label-checkbox">{{ $value }}</label>
      @endforeach
  ...
  <div class="form-group {{ $errors->has('client_arrange') ? 'has-error' : '' }}">
    ...
    <div class="col-md-6">
      @foreach ($client_arrange as $key => $value)
        ...
        <label for="client-arrange-{{ $key }}" class="label-checkbox">{{ $value }}</label>
      @endforeach
  ...
  <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
    ...
    <div class="col-md-6">
      ...
      <img src="{{ $project->display_image }}" alt="" width="300" id="project_image">
      @if ($errors->has('image'))
  ...
  <div class="form-group">
    <div class="col-md-6 col-md-offset-3">
      <button class="btn btn-primary" type="submit">@lang('admin.projects.edit_submit')</button>
    </div>
  </div>
  {{ Form::close() }}
</div>
...
@if ($project->duedate_at)
  $('#delivery_date').datepicker('setDate', '{{ $project->duedate_at->format('Y/m/d') }}');
@endif
</script>
@endpush
